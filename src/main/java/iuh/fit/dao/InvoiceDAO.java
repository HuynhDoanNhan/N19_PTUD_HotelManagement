...
public class InvoiceDAO {
	...
	public static Invoice getInvoiceByInvoiceID(String invoiceID) {
		...
		try (Connection connection = DBHelper.getConnection();
			...
			if (rs.next()) {
				invoice = new Invoice(rs.getString(1), ConvertHelper.localDateTimeConverter(rs.getTimestamp(2)),
	...
	public static void roomCheckingOut(String reservationFormID, String employeeID, double roomCharge,
	...
	public static void roomCheckingOutEarly(String reservationFormID, String employeeID, double roomCharge,
			double serviceCharge) {
	...
	private static Invoice extractData(ResultSet rs) throws SQLException {
		...
		// Thiết lập các thông tin khác liên quan đến ReservationForm, Employee,
		// Customer, Room, RoomCategory...
	...
	public static double getTotalRevenue() {
		...
		String sql = "SELECT SUM(netDue) FROM Invoice";
	...
	public static List<Invoice> getPaidInvoices() {
	    ...
	    String sql = """
	            SELECT invoiceID, invoiceDate, netDue
	            FROM Invoice 
	            WHERE netDue > 0
	            """;
	    ...
	    try (Connection con = DBHelper.getConnection();
	        ...
	        while (rs.next()) {
	            ...
	            inv.setNetDue(rs.getDouble("netDue"));
	            data.add(inv);
	        }
