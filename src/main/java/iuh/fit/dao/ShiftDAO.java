...
//    }
    ...
    public static Shift getCurrentShiftForLogin(Employee employee) {
        ...
        // Nhóm ngày hôm nay & hôm qua (MON_WEB_FRI, TUE_THU_SAT, SUNDAY)
        String todaySchedule = currentDaysScheduleToSQLConverter(now);
        String yesterdaySchedule = currentDaysScheduleToSQLConverter(now.minusDays(1));

        String sql = "SELECT s.shiftID, s.startTime, s.numberOfHour, s.shiftDaysSchedule " +
                "FROM Employee e " +
                "INNER JOIN ShiftAssignment b ON e.employeeID = b.employeeID " +
                "INNER JOIN Shift s ON b.shiftID = s.shiftID " +
                "WHERE e.employeeID = ? AND s.shiftDaysSchedule IN (?, ?)";
        ...
        try (
            ...
            statement.setString(1, employee.getEmployeeID());
            statement.setString(2, todaySchedule);
            statement.setString(3, yesterdaySchedule);

            try (ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
                    LocalTime startTime = ConvertHelper.localTimeConverter(rs.getTime("startTime"));
                    int numberOfHour = rs.getInt("numberOfHour");
                    LocalTime endTime = startTime.plusHours(numberOfHour);

                    boolean isOvernight = endTime.isBefore(startTime);   // ví dụ 23:00–05:00
                    String shiftSchedule = rs.getString("shiftDaysSchedule");

                    boolean isToday = shiftSchedule.equals(todaySchedule);
                    boolean isYesterday = shiftSchedule.equals(yesterdaySchedule);
                    ...
                    if (!isOvernight) {
                        ...
                        // Ca qua đêm (ví dụ 23:00–05:00)
                        if (isToday) {
                            // Ngày bắt đầu ca: từ startTime -> 23:59
                            if (!nowTime.isBefore(startTime)) {
                                isInShift = true;
                            }
                        } else if (isYesterday) {
                            // Ngày sau: từ 00:00 -> endTime
                    ...
                    if (isInShift) {
                        currentShift = ShiftDAO.getDataByID(rs.getString("shiftID"));
                        break;
                    }
                }
            }
